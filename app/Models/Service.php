...
class Service extends Model
{
    ...
    private static $translates_class = 'App\Models\ServicesTranslate';
    private static $main_table = 'services';
    private static $translates_table = 'services_translates';
    private static $gallery_images_table = 'services_photos';
    ...
    {
        return $this->hasOne('App\Models\ServicesTranslate','parent_id')->where('lang',locale());
    }

    public function images()
    {
        return $this->hasMany('App\Models\ServicesPhoto', 'parent_id');
    }
    ...
    public static function addItem($request)
    {
        ...
        $request_keys = $request->except(['_token','translates','image','status','main_page']);

        $item->status = $request->status === 'on' ? 1 : 0;
        $item->main_page = $request->main_page === 'on' ? 1 : 0;
    ...
    public static function updateItem($request, $item)
    {
        ...
        if ($request->hasFile('image'))
        {
            ...
            *
            */
            if(file_exists('../public_html' . $item->image) && !is_null($item->image))
            {
